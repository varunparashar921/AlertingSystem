...
mongoose.connect('mongodb://localhost:27017/tsunami', { useMongoClient: true });
var app = express();
...
db.on('connected', function () {
	console.log('Mongoose Tsunami alertingsystem connection opend');
})

// If the connection throws an error
db.on('error', function (err) {
	console.log('Mongoose Tsunami alertingsystem connection error: ' + err);
})

// When the connection is disconnected
db.on('disconnected', function () {
	console.log('Mongoose Tsunami alertingsystem connection disconnected');
})

// Data
app.get('/tsunami/getData', function (req, res) {
...
// register Alert
app.post('/tsunami/saveAlert', function (req, res) {
...
var server = app.listen(port, function () {
    ...
    console.log("Tsumani app listening at http://%s:%s", host, port)
})
